...
(function() {
  ...
  Sersic = (function(_super) {
    ...
    Sersic.prototype.getParamList = function() {
      return ['center_x', 'center_y', 'angle', 'axisRatio', 'effRadius', 'intensity', 'n'];
    };
    ...
    Sersic.prototype.generate = function() {
      ...
      for (x = _i = 0, _ref = this.width - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; x = 0 <= _ref ? ++_i : --_i) {
        for (y = _j = _ref1 = this.height - 1; _ref1 <= 0 ? _j <= 0 : _j >= 0; y = _ref1 <= 0 ? ++_j : --_j) {
          r_x = (x - this.params.center.x) * cos + (y - this.params.center.y) * sin;
          r_y = ((y - this.params.center.y) * cos - (x - this.params.center.x) * sin) / this.params.axisRatio;
